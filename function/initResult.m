function [ TotalResult ] = initResult( CusPos,SatPos,Cus2CusDist,Sat2CusDist,Sat2SatDist,Center2SatDist,Demand,QUESTIONOpts,ifTryEmpty,emptySatId )
TotalResult=cell(1,4);

satNum=size(SatPos,1);
%if try empty sat
if  nargin>8
    if ifTryEmpty==1
        TempSat2CusDist=Sat2CusDist;
        TempSat2CusDist(emptySatId,:)=+inf;
        TotalSatPos=distBasedSatCusDivide(TempSat2CusDist,CusPos); %cell index:satId, column1:SatCusPos
        %fprintf('cusNum=%d\n',size(TotalSatPos{1,1},1));\
    end
else
...
for i=1:QUESTIONOpts.Satellites %for every sat
    ...
    if size(TotalSatPos{i,1},1)==0
        continue 
    end
    [dividedResult,ifSuccess]=degreeDivide(TotalSatPos{i,1},SatPos(i,:),Demand,QUESTIONOpts.L2Fleet,QUESTIONOpts.L2Capacity); %1:CusId 2:PartId
    if ~ifSuccess
...
while size(nonzeros(SatLoad),1)>0
    ...
    CandidateSat=find(SatLoad~=0);minIndex=CandidateSat(1); % min dist from center or last sat
